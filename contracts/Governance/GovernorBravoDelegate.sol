...
contract GovernorBravoDelegate is GovernorBravoDelegateStorageV2, GovernorBravoEvents {
    ...
    bytes32 public immutable BALLOT_TYPEHASH = keccak256("Ballot(uint256 proposalId,uint8 support)");
    ...
    /**
      ...
      */
    function initialize(address timelock_, address comp_, uint votingPeriod_, uint votingDelay_, uint proposalThreshold_) virtual public onlyAdmin {
        require(address(timelock) == address(0), "Can only initialize once");
        require(timelock_ != address(0), "Invalid timelock address");
        require(comp_ != address(0), "Invalid comp address");
        ...
        require(proposalThreshold_ >= MIN_PROPOSAL_THRESHOLD && proposalThreshold_ <= MAX_PROPOSAL_THRESHOLD, "Invalid proposal threshold");
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        ...
        require(comp.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold || isWhitelisted(msg.sender), "Votes below proposal threshold");
        ...
        require(newProposal.id == 0, "ProposalID collsion");
    ...
    function queue(uint proposalId) external {
        ...
        uint len = proposal.targets.length;
        for (uint i; i < len;) {
    ...
    function queueOrRevertInternal(address target, uint value, string memory signature, bytes memory data, uint eta) internal {
        require(!timelock.queuedTransactions(keccak256(abi.encode(target, value, signature, data, eta))), "Identical action already queued");
    ...
    function cancel(uint proposalId) external {
        ...
        if(msg.sender != proposal.proposer) {
            ...
            if(isWhitelisted(proposal.proposer)) {
                require((comp.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold) && msg.sender == whitelistGuardian, "Whitelisted proposer");
            }
            else {
                require((comp.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold), "Proposer above threshold");
            }
        }
    ...
    /**
      ...
      */
    function _setVotingDelay(uint newVotingDelay) external onlyAdmin {
    ...
    /**
      ...
      */
    function _setVotingPeriod(uint newVotingPeriod) external onlyAdmin {
    ...
    /**
      ...
      */
    function _setProposalThreshold(uint newProposalThreshold) external onlyAdmin {
    ...
    function _setWhitelistAccountExpiration(address account, uint expiration) external {
        require(msg.sender == admin || msg.sender == whitelistGuardian, "Only admin or whitelistGuardian");
     ...
     */
     function _setWhitelistGuardian(address account) external onlyAdmin {
    ...
    function _initiate(address governorAlpha) external onlyAdmin {
        require(initialProposalId == 0, "Can only initiate once");
